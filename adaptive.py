...
###FFH updated, 03.11.2018: Changed CNN model 
class AttentiveCNN(nn.Module):
    def __init__(self, embed_size, hidden_size):
        ...
        #####change into densenet
        resnet = models.densenet201(pretrained=True)
        # print "densnet children",len(list( resnet.children() ))
        #####if is resnet152 need to delete last two layer 
        modules = list(resnet.children())[:-1]  # delete the last fc layer.
        resnet_conv = nn.Sequential(*modules)  # last conv feature

        self.resnet_conv = resnet_conv
        self.avgpool = nn.AvgPool2d(7)
        ###### Need to update according to cnn models. ResNet152->2480
        self.affine_a = nn.Linear(1920, hidden_size)  # v_i = W_a * A
        self.affine_b = nn.Linear(1920, embed_size)  # v_g = W_b * a^g
...
class Atten(nn.Module):
    def __init__(self, hidden_size):
        ...
        self.affine_v = nn.Linear(hidden_size, 85, bias=False)  # W_v
        self.affine_g = nn.Linear(hidden_size, 85, bias=False)  # W_g
        self.affine_s = nn.Linear(hidden_size, 85, bias=False)  # W_s
        self.affine_h = nn.Linear(85, 1, bias=False)  # w_h

        self.affine_vd = nn.Linear(hidden_size, 85, bias=False)  # W_vd
        self.affine_gd = nn.Linear(hidden_size, 85, bias=False)  # W_gd
        self.affine_hd = nn.Linear(85, 1, bias=False)  # w_hc
    ...
    def init_weights(self):
        ...
        init.xavier_uniform(self.affine_vd.weight)
        init.xavier_uniform(self.affine_gd.weight)
        init.xavier_uniform(self.affine_hd.weight)
...
class AdaptiveBlock(nn.Module):
    def __init__(self, embed_size, hidden_size, vocab_size):
        ...
        self.atten_loc = Atten(hidden_size)
    ...
    def forward(self, x, hiddens, cells, V, V_d):
        ...
        # get C_td, location attention, sentinel score
        cd_hat, atten_weights_d, beta_d = self.atten_loc(V_d, hiddens, sentinel)

        # Final score along vocabulary
        scores = self.mlp(self.dropout(c_hat + hiddens))
        scores_d = self.mlp(self.dropout(cd_hat + hiddens))
        scores_com = scores + scores_d

        return scores_com, atten_weights, atten_weights_d, beta
...
class Decoder(nn.Module):
    ...
    def forward(self, V, V_d, v_g, captions, states=None):
        ...
        if torch.cuda.device_count() > 1:
            ...
            scores, atten_weights, atten_weights_d, beta = adaptive_block_parallel(x, hiddens, cells, V)
        else:
            scores, atten_weights, atten_weights_d, beta = self.adaptive(x, hiddens, cells, V, V_d)

        # Return states for Caption Sampling purpose
        return scores, states, atten_weights, atten_weights_d, beta
...
class Encoder2Decoder(nn.Module):
    def __init__(self, embed_size, vocab_size, hidden_size):
        ...
        self.conv = nn.Conv1d(2054, hidden_size, 1)  # 2048=> 2054(add location info)
        self.init_weights()

    def init_weights(self):
        init.normal(self.conv.weight, 0, 0.01)

    def forward(self, images, captions, lengths, det_features):
        ...
        # apply 1d conv to det_features, convert shape 36*2048 -> 36*hidden_size
        # then concat with V
        Vd = det_features.transpose(1, 2)  # N, 2048, 36
        Vd = self.conv(Vd)  # N, hidden, 36
        Vd = Vd.transpose(1, 2)  # N, 36, hidden
        # V = torch.cat((V, Vd), dim=1)  # N, 85, hidden

        # Language Modeling on word prediction
        scores, _, _, _ = self.decoder(V, Vd, v_g, captions)
    ...
    # Caption generator
    def sampler(self, images, det_features, max_len=20):
        """
        ...
        # apply 1d conv to det_features, convert shape 36*2048 -> 36*1920
        # then concat with V
        Vd = det_features.transpose(1, 2)  # N, 2048, 36
        Vd = self.conv(Vd)  # N, 1920, 36
        Vd = Vd.transpose(1, 2)  # N, 36, 1920
        #V = torch.cat((V, Vd), dim=1)  # N, 85, 1920
        ...
        for i in range(max_len):
            scores, states, atten_weights, atten_weights_d, beta = self.decoder(V, Vd, v_g, captions, states)
            predicted = scores.max(2)[1]  # argmax
            captions = predicted

            atten_all = torch.cat((atten_weights, atten_weights_d), dim=2)
            # Save sampled word, attention map and sentinel at each timestep
            sampled_ids.append(captions)
            attention.append(atten_all)
            Beta.append(beta)

        # caption: B x max_len
        # attention: B x max_len x 85
